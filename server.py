...
def get_target(cmd):
    try:
        target = cmd.replace('select ', '')  # Extract target ID
        target = int(target)

        if target < 0 or target >= len(all_connection):  # Ensure target index is valid
            print("Selection not valid")
            return None

        conn = all_connection[target]
        print(f"You are now connected to: {all_address[target][0]}")
        print(f"{all_address[target][0]}>", end="")  # Proper syntax

        return conn  # Return the connection to be used in send_target_commands()
